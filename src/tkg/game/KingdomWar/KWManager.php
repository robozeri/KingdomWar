...
class KWManager  extends MiniGameBase  {
	/*
	 * KW Commands
	 */
	const KW_COMMAND = "kw";
	const KW_COMMAND_HOME = "home";
	const KW_COMMAND_JOIN = "join";
	const KW_COMMAND_JOIN_RED_TEAM = "joinred";
	const KW_COMMAND_JOIN_BLUE_TEAM = "joinblue";
	const KW_COMMAND_LEAVE = "leave";
	const KW_COMMAND_START = "start";
	const KW_COMMAND_STOP = "stop";
	const KW_COMMAND_STATS = "stats";
	const KW_COMMAND_CREATE_ARENA = "create";
	const KW_COMMAND_RESET_ARENA = "reset";
	const KW_COMMAND_BLOCK_DISPLAY_ON = "blockon";
	const KW_COMMAND_BLOCK_DISPLAY_OFF = "blockoff";
	//change sign location
	const KW_COMMAND_SET_SIGN_JOIN_BLUE_TEAM = "setsignblue";
	const KW_COMMAND_SET_SIGN_JOIN_RED_TEAM = "setsignred";
	const KW_COMMAND_SET_SIGN_VIEW_STATS = "setsignstats";
	const KW_COMMAND_SET_SIGN_NEW_GAME = "setsignnew";
	//change block type
	const KW_COMMAND_SETBLOCK_ID_TEAM_BORDER = "setblockborder";
	const KW_COMMAND_SETBLOCK_ID_DEFENCE_WALL_BLUE_TEAM = "setblockwallblue";
	const KW_COMMAND_SETBLOCK_ID_DEFENCE_WALL_RED_TEAM = "setblockwallred";	
					
	/*
	 * KW permissions 
	 */
	const KW_PERMISSION_ROOT = "tkg.kw";
	const KW_PERMISSION_COMMANDS = "tkg.kw";	
	const KW_PERMISSION_HOME = "tkg.kw.command.home";
	const KW_PERMISSION_START = "tkg.kw.command.start";
	const KW_PERMISSION_STOP = "tkg.kw.command.stop";
	const KW_PERMISSION_LEAVE = "tkg.kw.command.leave";
	const KW_PERMISSION_CREATE_ARENA = "tkg.kw.command.create";
	const KW_PERMISSION_RESET_ARENA = "tkg.kw.command.reset";
	const KW_PERMISSION_STATS = "tkg.kw.command.stats";	
	const KW_PERMISSION_JOIN_BLUE_TEAM = "tkg.kw.command.joinblue";						
	const KW_PERMISSION_JOIN_RED_TEAM = "tkg.kw.command.joinred";
	const KW_PERMISSION_BLOCK_DISPLAY_ON = "tkg.kw.command.blockon";
	const KW_PERMISSION_BLOCK_DISPLAY_OFF = "tkg.kw.command.blockoff";
	...
	public function onCommand(CommandSender $sender, Command $command, $label, array $args) {
		...
		if (! $sender instanceof PLayer) {
			$sender->sendMessage ( $this->getMsg ( "kw.error.wrong-sender" ) );
			return;
		}
		$player = $sender->getPlayer ();
		if ((strtolower ( $command->getName () ) == self::CTF_COMMAND) && isset ( $args [0] )) {
			if (strtolower ( $args [0] ) == self::KW_COMMAND_CREATE_ARENA || strtolower ( $args [0] ) == self::KW_COMMAND_RESET_ARENA) {
				if (! $sender->isOp ()) {
					$sender->sendMessage ( $this->getMsg ( "kw.error.no-permission" ) );
					return;
				}
				...
				return true;
			} elseif (strtolower ( $args [0] ) == self::kw_COMMAND_BLOCK_DISPLAY_ON) {
				if (! $sender->isOp ()) {
					$sender->sendMessage ( $this->getMsg ( "kw.error.no-permission" ) );
					return;
				}
			...
			} elseif (strtolower ( $args [0] ) == self::KW_COMMAND_JOIN_BLUE_TEAM) {
				$sender->sendMessage ( $this->getMsg ( "team.join-blue" ) );
				$this->handleJoinBlueTeam ( $player );
				return;
			} elseif (strtolower ( $args [0] ) == self::KW_COMMAND_JOIN_RED_TEAM) {
				$sender->sendMessage ( $this->getMsg ( "team.join-red" ) );
				$this->handleJoinRedTeam ( $player );
				return;
			} elseif (strtolower ( $args [0] ) == self::KW_COMMAND_LEAVE) {
				$sender->sendMessage ( $this->getMsg ( "game.player-left" ) );
				$this->handleLeaveTheGame ( $player );
			} elseif (strtolower ( $args [0] ) == self::KW_COMMAND_STOP) {
				$sender->sendMessage ( $this->getMsg ( "game.player-stop" ) );
				$this->stopGame($player );
			} elseif (strtolower ( $args [0] ) == self::KW_COMMAND_HOME) {
				$sender->sendMessage ( $this->getMsg ( "sign.teleport.kw" ) );
				$this->teleportPlayerToHome ( $player );
			} elseif (strtolower ( $args [0] ) == self::KW_COMMAND_START) {
				$sender->sendMessage ( $this->getMsg ( "game.player-start" ) );
				$this->handleStartTheGame ( $player->getLevel () );
			} elseif (strtolower ( $args [0] ) == self::KW_COMMAND_STATS) {	
				$sender->sendMessage ( $this->getMsg ( "game.stats" ) );
				$this->displayTeamScores ( $sender );
			} elseif (strtolower ( $args [0] ) == self::KW_COMMAND_SET_SIGN_JOIN_BLUE_TEAM) {
				if (! $sender->isOp ()) {
					$sender->sendMessage ( $this->getMsg ( "kw.error.no-permission" ) );
					return;
				}
				$this->getPlugIn()->setupModeAction= self::KW_COMMAND_SET_SIGN_JOIN_BLUE_TEAM;				
				$sender->sendMessage ($this->getMsg ( "kw.setup.action" ).self::KW_COMMAND_SET_SIGN_JOIN_BLUE_TEAM);
				$sender->sendMessage($this->getMsg ( "kw.setup.select" ));				
			} elseif (strtolower ( $args [0] ) == self::KW_COMMAND_SET_SIGN_JOIN_RED_TEAM) {
				if (! $sender->isOp ()) {
					$sender->sendMessage ( $this->getMsg ( "kw.error.no-permission" ) );
					return;
				}
				$this->getPlugIn()->setupModeAction= self::KW_COMMAND_SET_SIGN_JOIN_RED_TEAM;
				$sender->sendMessage ($this->getMsg ( "kw.setup.action" ).self::KW_COMMAND_SET_SIGN_JOIN_RED_TEAM);
				$sender->sendMessage($this->getMsg ( "kw.setup.select" ));
			} elseif (strtolower ( $args [0] ) == self::KW_COMMAND_SET_SIGN_NEW_GAME) {
				if (! $sender->isOp ()) {
					$sender->sendMessage ( $this->getMsg ( "kw.error.no-permission" ) );
					return;
				}
				$this->getPlugIn()->setupModeAction= self::KW_COMMAND_SET_SIGN_NEW_GAME;
				$sender->sendMessage ($this->getMsg ( "kw.setup.action" ).self::KW_COMMAND_SET_SIGN_NEW_GAME);
				$sender->sendMessage($this->getMsg ( "kw.setup.select" ));
			} elseif (strtolower ( $args [0] ) == self::KW_COMMAND_SET_SIGN_VIEW_STATS) {
				if (! $sender->isOp ()) {
					$sender->sendMessage ( $this->getMsg ( "kw.error.no-permission" ) );
					return;
				}
				$this->getPlugIn()->setupModeAction= self::KW_COMMAND_SET_SIGN_VIEW_STATS;
				$sender->sendMessage ($this->getMsg ( "kw.setup.action" ).self::KW_COMMAND_SET_SIGN_VIEW_STATS);
				$sender->sendMessage($this->getMsg ( "kw.setup.select" ));
				
			}elseif (strtolower ( $args [0] ) == self::KW_COMMAND_SETBLOCK_ID_TEAM_BORDER) {
				if (! $sender->isOp ()) {
					$sender->sendMessage ( $this->getMsg ( "kw.error.no-permission" ) );
					return;
				}
				$this->getPlugIn()->setupModeAction= self::KW_COMMAND_SETBLOCK_ID_TEAM_BORDER;
				$sender->sendMessage ($this->getMsg ( "kw.setup.action" ).self::KW_COMMAND_SETBLOCK_ID_TEAM_BORDER);
				$sender->sendMessage($this->getMsg ( "kw.setup.select" ));
				
			} elseif (strtolower ( $args [0] ) == self::KW_COMMAND_SETBLOCK_ID_DEFENCE_WALL_BLUE_TEAM) {
				if (! $sender->isOp ()) {
					$sender->sendMessage ( $this->getMsg ( "kw.error.no-permission" ) );
					return;
				}
				$this->getPlugIn()->setupModeAction= self::KW_COMMAND_SETBLOCK_ID_DEFENCE_WALL_BLUE_TEAM;
				$sender->sendMessage ($this->getMsg ( "kw.setup.action" ).self::KW_COMMAND_SETBLOCK_ID_DEFENCE_WALL_BLUE_TEAM);
				$sender->sendMessage($this->getMsg ( "kw.setup.select" ));
				
			} elseif (strtolower ( $args [0] ) == self::KW_COMMAND_SETBLOCK_ID_DEFENCE_WALL_RED_TEAM) {
				if (! $sender->isOp ()) {
					$sender->sendMessage ( $this->getMsg ( "kw.error.no-permission" ) );
					return;
				}
				$this->getPlugIn()->setupModeAction= self::KW_COMMAND_SETBLOCK_ID_DEFENCE_WALL_RED_TEAM;
				$sender->sendMessage ($this->getMsg ( "kw.setup.action" ).self::KW_COMMAND_SETBLOCK_ID_DEFENCE_WALL_RED_TEAM);
				$sender->sendMessage($this->getMsg ( "kw.setup.select" ));
			}
	...
	public function stopGame(Player $player) {
		...
		if (!isset($inGamePlayers[$player->getName()])) {
			...
			$player->sendMessage ( $this->getMsg ( "kw.error.not-game-stop" ) );
			return;
		}				
		// display
		...
		$arenaPos = $this->getSetup ()->getGamePos ( CTFSetup::KW_GAME_ARENA_POSITION );
	...
	public function displayTeamScores(Player $player) {
		...
		$output .= TextFormat::DARK_RED.$this->getMsg ( "kw.name" ) . " | " . TextFormat::DARK_GREEN.$this->getMsg ( "kw.status" ) . ": " . $status . "\n";
		$output .= TextFormat::DARK_GRAY."------------------------------\n";
	...
	public function onClickNewGameSign(Player $player, $blockTouched) {
		$signNewPos = $this->getSetup ()->getSignPos ( KWSetup::CLICK_SIGN_NEW_GAME );
	...
	public function updateGameSigns(Player $player) {
		...
		if ($tile!=null) {
			$tile->setText ( TextFormat::RED . "KW ", TextFormat::GOLD ."Start New Game", TextFormat::WHITE . "Reset Arena" , TextFormat::GOLD . "[Tap]" );
		}
		
		$statSignPos = $this->getSetup ()->getSignPos ( KWSetup::CLICK_SIGN_SHOW_GAME_STAT );
		$tile = $player->level->getTile( new Position($statSignPos->x, $statSignPos->y, $statSignPos->z, $player->level));
		if ($tile!=null) {
			$tile->setText ( TextFormat::RED . "KW ", TextFormat::GOLD ."Play Stats", TextFormat::WHITE . "View" , TextFormat::GOLD . "[Tap]" );
		}
	}
	
	public function updateBlueTeamSign(Player $player) {
		$joinBluePos = $this->getSetup ()->getSignPos ( KWSetup::CLICK_SIGN_JOIN_BLUE_TEAM );
		$tile = $player->level->getTile( new Position($joinBluePos->x, $joinBluePos->y, $joinBluePos->z, $player->level));
		$maxTeamPlayers = $this->getSetup ()->getMaxPlayerPerTeam ();
		if ($tile!=null) {
			$tile->setText ( TextFormat::RED . "KW ", TextFormat::BLUE ."BLUE TEAM", TextFormat::WHITE . "JOIN NOW"  , TextFormat::GOLD . "[" . count ( $this->plugin->blueTeamPLayers )."/".$maxTeamPlayers . "]");
		}
	}
	
	public function updateRedTeamSign(Player $player) {
		$joinRedPos = $this->getSetup ()->getSignPos ( KWSetup::CLICK_SIGN_JOIN_RED_TEAM );
		$tile = $player->level->getTile( new Position($joinRedPos->x, $joinRedPos->y, $joinRedPos->z, $player->level));
		$maxTeamPlayers = $this->getSetup ()->getMaxPlayerPerTeam ();
		if ($tile!=null) {
			$tile->setText ( TextFormat::RED . "KW ", TextFormat::RED ."RED TEAM", TextFormat::WHITE . "JOIN NOW"  , TextFormat::GOLD . "[" . count ( $this->plugin->redTeamPlayers )."/".$maxTeamPlayers. "]");
		}
	}
	...
	public function handleNewGame(Player $player) {
		...
		// reset close gates
		$arenaSize = $this->getSetup ()->getArenaSize ();
		$arenaPos = $this->getSetup ()->getGamePos ( KWSetup::KW_GAME_ARENA_POSITION );
		// close gate
	...
	public function checkRedTeamCapturedEnermyFlag(Player $player, Level $level, $rb) {
		$redTeamEnemyFlagPos = $this->getSetup ()->getFlagPos ( KWSetup::KW_FLAG_RED_TEAM_ENEMY );
		...
		if (round ( $rb->x ) == round ( $redTeamEnemyFlagPos->x ) && round ( $rb->y ) == round ( $redTeamEnemyFlagPos->y ) && round ( $rb->z ) == round ( $redTeamEnemyFlagPos->z )) {
			...
			if ($blueflag->getId () != 0) {
				$player->sendMessage (TextFormat::YELLOW. $this->getMsg ( "kw.error.blueteam-flag-exist" ) );
				// erase placed block
			...
			$output .= TextFormat::GREEN.$this->getMsg ( "kw.conglatulations" ) . "\n";
			$output .= TextFormat::RED.$this->getMsg ( "kw.red-team.capturedflag" ) . "\n";
			$output .= TextFormat::GRAY."------------------------------\n";
			// update blue team winning score
			$this->getPlugIn ()->redTeamWins ++;
			$output .= TextFormat::LIGHT_PURPLE.$this->getMsg ( "kw.blue-team.score" ) . $this->getPlugIn ()->blueTeamWins . "\n";
			$output .= TextFormat::LIGHT_PURPLE.$this->getMsg ( "kw.red-team.score" ) . $this->getPlugIn ()->redTeamWins . "\n";
			$output .= TextFormat::GRAY."------------------------------\n";
	...
	public function checkBlueTeamCapturedEnermyFlag(Player $player, Level $level, $rb) {
		$blueTeamEnemyFlagPos = $this->getSetup ()->getFlagPos ( KWSetup::KW_FLAG_BLUE_TEAM_ENEMY );
		if (round ( $rb->x ) == round ( $blueTeamEnemyFlagPos->x ) && round ( $rb->y ) == round ( $blueTeamEnemyFlagPos->y ) && round ( $rb->z ) == round ( $blueTeamEnemyFlagPos->z )) {
			// check if enermy flag is destroyed
			$blueTeamFlagPos = $this->getSetup ()->getFlagPos ( KWSetup::KW_FLAG_RED_TEAM );
			$redflag = $level->getBlock ( $blueTeamFlagPos );
			if ($redflag->getId () != 0) {
				$player->sendMessage ( $this->getMsg ( "kw.error.redteam-flag-exist" ) );
				// erase placed block
				$this->getBuilder ()->resetBlock ( $rb, $level, 0 );
				return;
			}
			...
			$output .=TextFormat::GRAY. "------------------------------\n";
			$output .= TextFormat::GREEN.$this->getMsg ( "kw.conglatulations" ) . "\n";
			$output .= TextFormat::BLUE.$this->getMsg ( "kw.blue-team.capturedflag" ) . "\n";
			$output .= TextFormat::GRAY."------------------------------\n";
			// update blue team winning score
	...
	public function handleNextRoundOfGame(Level $level) {
		...
		if ($this->getPlugIn ()->currentGameRound < $this->getPlugIn ()->maxGameRound) {
			...
			$this->getPlugIn ()->getServer ()->broadcastMessage ( $this->getMsg ( "kw.finished" ) );
			$this->handleStopTheGame ();
		}
	}
	...
	public function handleJoinBlueTeam(Player $player) {
		$blueTeamEntryPos = $this->getSetup ()->getGamePos ( KWSetup::KW_GAME_ARENA_POSITION_ENTRY_BLUE_TEAM );		
		...
		$this->getGameKit ()->putOnGameKit ( $player, KWGameKit::KIT_BLUE_TEAM );
	...
	public function handleJoinRedTeam(Player $player) {
		$redTeamEntryPos = $this->getSetup ()->getGamePos ( KWSetup::KW_GAME_ARENA_POSITION_ENTRY_RED_TEAM );
		...
		$this->getGameKit ()->putOnGameKit ( $player, KWGameKit::KIT_RED_TEAM );
		$player->sendMessage ( TextFormat::GRAY.$this->getMsg ( "team.tap-start" ) );
	...
	public function handleLeaveTheGame(Player $player) {
		...
		$gameWaitingRoomPos = $this->getSetup ()->getGamePos ( KWSetup::KW_GAME_ARENA_POSITION_WAITING_ROOM );
		$player->teleport ( $gameWaitingRoomPos );
		
		$player->getServer ()->broadcastMessage ( $player->getName () . $this->getMsg ( "kw.left-game" ), $this->plugin->blueTeamPLayers);
		$player->getServer ()->broadcastMessage ( $player->getName () . $this->getMsg ( "kw.left-game" ) , $this->plugin->redTeamPlayers);
	}
	...
	public function handleStopTheGame() {
		...
		$waitingRoomPos = $this->getSetup ()->getGamePos ( KWSetup::KW_GAME_ARENA_POSITION_WAITING_ROOM );
		foreach ( $this->getPlugIn ()->redTeamPlayers as $rp ) {
			...
			$rp->sendMessage ( TextFormat::GRAY.$this->getMsg ( "kw.return-waiting-area" ) );
		...
		foreach ( $this->getPlugIn ()->blueTeamPLayers as $bp ) {
			...
			$bp->sendMessage ( TextFormat::GRAY.$this->getMsg ( "kw.return-waiting-area" ) );
	...
	public function moveTeamBackToSpawnpointOnCapturedEnermyFlag() {
		// send all players to waiting room
		$redTeamSpawnPos = $this->getSetup ()->getGamePos ( KWSetup::KW_GAME_ARENA_POSITION_ENTRY_RED_TEAM );
		foreach ( $this->getPlugIn ()->redTeamPlayers as $rp ) {
			$rp->teleport ( $redTeamSpawnPos );
		}
		$blueTeamSpawnPos = $this->getSetup ()->getGamePos ( KWSetup::KW_GAME_ARENA_POSITION_ENTRY_BLUE_TEAM );
		foreach ( $this->getPlugIn ()->blueTeamPLayers as $bp ) {
	...
	public function teleportPlayerToHome(Player $player) {
		...
		if ($player->getServer ()->isLevelLoaded ( $levelhome )) {
			...
			$ctfGamePos =TextFormat::GRAY. $this->getSetup ()->getGamePos ( KWSetup::KW_GAME_ENTRY );
			$level->getChunk($kwGamePos->x, $kwGamePos->z);
	...
	public function buildGameArena(CommandSender $sender) {
		...
		$arenaPos = $this->getSetup ()->getGamePos ( KWSetup::KW_GAME_ARENA_POSITION );
		if (! $sender instanceof Player) {
			$sender->sendMessage ( TextFormat::YELLOW.$this->getMsg ( "kw.error.wrong-sender" ) );
			return;
		}
	...
	public function onClickStartGameButton($level, Player $player, $blockTouched) {
		$startButtonPos = $this->getSetup ()->getButtonPos ( KWSetup::CLICK_BUTTON_START_GAME );
		// START BUTTON
	...
	public function onClickStopGameButton($level, Player $player, $blockTouched) {
		$stopButtonPos = $this->getSetup ()->getButtonPos ( KWSetup::CLICK_BUTTON_STOP_GAME );
		// STOP BUTTON
	...
	public function onClickLeaveGameButton($level, $player, $blockTouched) {
		$leaveButtonPos = $this->getSetup ()->getButtonPos ( KWSetup::CLICK_BUTTON_LEAVE_GAME );
		// LEAVE BUTTON
	...
	public function handlePlayerQuit(Player $player) {
		...
		if (isset ( $this->getPlugIn ()->redTeamPlayers [$player->getName ()] )) {
			...
			if ($player->getInventory ()->contains ( new Item ( Item::CARPET ) )) {
				...
				$msg = TextFormat::GRAY.$player->getName () . " [" . $this->getMsg ( "kw.return-flag" ) . "]";
				$player->getServer ()->broadcastMessage ( $msg );
		...
		if (isset ( $this->getPlugIn ()->blueTeamPlayers [$player->getName ()] )) {
			...
			if ($player->getInventory ()->contains ( new Item ( Item::CARPET ) )) {
				// put this flag back to team
				$msg = TextFormat::WHITE.$player->getName () . " [" . $this->getMsg ( "kw.return-flag" ) . "]";
				//$player->getServer ()->broadcastMessage ( $msg );
				$player->getServer ()->broadcastMessage ( $msg, $this->getPlugIn ()->blueTeamPlayers);
				$player->getServer ()->broadcastMessage ( $msg, $this->getPlugIn ()->redTeamPlayers);
	...
	/**
	 * Handle player entry to KW game world
	 *
	 * @param Player $player        	
	 */
	public function handlePlayerEntry(Player $player) {
		...
		$gameWorld = $this->getSetup ()->getKWWorldName ();
		if (strtolower ( $player->level->getName () ) == strtolower ( $gameWorld )) {
			// send entry point
			$gameWorldPos = $this->getSetup ()->getGamePos ( KWSetup::KW_GAME_ENTRY );
	...
	private function grantPlayerDefaultPermissions(Player $player) {
		$player->addAttachment($this->getPlugIn(),self::KW_PERMISSION_HOME, TRUE);
		$player->addAttachment($this->getPlugIn(),self::KW_PERMISSION_JOIN_BLUE_TEAM, TRUE);		
		$player->addAttachment($this->getPlugIn(),self::KW_PERMISSION_JOIN_RED_TEAM, TRUE);		
		$player->addAttachment($this->getPlugIn(),self::KW_PERMISSION_STATS, TRUE);
		$player->addAttachment($this->getPlugIn(),self::KW_PERMISSION_LEAVE, TRUE);
		$player->addAttachment($this->getPlugIn(),self::KW_PERMISSION_START, TRUE);
		$player->addAttachment($this->getPlugIn(),self::KW_PERMISSION_STOP, TRUE);
		if ($player->isOp()) {
			$player->addAttachment($this->getPlugIn(),self::KW_PERMISSION_CREATE_ARENA, TRUE);
			$player->addAttachment($this->getPlugIn(),self::KW_PERMISSION_RESET_ARENA, TRUE);
			$player->addAttachment($this->getPlugIn(),self::KW_PERMISSION_BLOCK_DISPLAY_ON, TRUE);
			$player->addAttachment($this->getPlugIn(),self::KW_PERMISSION_BLOCK_DISPLAY_OFF, TRUE);
		}
	}
